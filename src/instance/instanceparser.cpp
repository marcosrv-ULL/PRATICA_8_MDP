...
InstanceParser::InstanceParser(std::string filename, int mmax) {
...
char InstanceParser::get() {
  return ((unsigned int)head < _string_program.length())
             ? _string_program[head++]
             : '@';
}
